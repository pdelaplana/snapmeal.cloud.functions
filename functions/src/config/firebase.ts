...
console.log(
	`Using environment variable DATABASE_ID: ${process.env.DATABASE_ID}`,
);
console.log(
	`Using environment variable STORAGE_BUCKET: ${process.env.STORAGE_BUCKET}`,
);
...
try {
	...
	if (fs.existsSync('./firebase-service-account.json')) {
		admin.initializeApp({
			...
			storageBucket:
				process.env.STORAGE_BUCKET || 'snapmeal-sa2e9.firebasestorage.app',
		});
		console.log(
			`Using service account from file: firebase-service-account.json`,
		);
	} else {
